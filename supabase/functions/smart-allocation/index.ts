...
async function getSupervisorPool(client: any, maxProjectsDefault: number) {
  const { data: supervisors, error: supervisorsError } = await client.from("supervisors").select("*");

  if (supervisorsError) throw supervisorsError;
  if (!supervisors?.length) return [];

  const supervisorIds = supervisors.map((supervisor: any) => supervisor.user_id);
  const { data: profiles, error: profilesError } = await client
    .from("profiles")
    .select("user_id, full_name, email, department, research_areas, max_projects, current_projects")
...
async function syncSupervisorProjectCount(client: any, supervisorId: string) {
  ...
  await Promise.all([
    client.from("supervisors").update({ current_projects: currentProjects }).eq("user_id", supervisorId),
    client.from("profiles").update({ current_projects: currentProjects }).eq("user_id", supervisorId),
  ]);
...
function hungarianAlgorithm(scoreMatrix: number[][]): [number, number][] {
  ...
  const cost: number[][] = Array.from({ length: size }, (_, i) =>
    Array.from({ length: size }, (_, j) => (i < n && j < m ? maxVal - scoreMatrix[i][j] : maxVal)),
  );
...
function scoreMatch(
  ...
  const projectKeywords = (
    project.keywords && project.keywords.length > 0 ? project.keywords : projectCategory ? [projectCategory] : []
...
function supervisorToDocument(supervisor: any): string[] {
  const parts = [...(supervisor.research_areas || []), supervisor.department || ""];
  return tokenize(parts.join(" "));
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "create_group" || action === "allocate_group" || action === "auto_allocate_project") {
      ...
      action === "accept_group_allocation" ||
      action === "reject_group_allocation" ||
      ...
      if (!["admin", "supervisor"].includes(profile.user_type)) {
        throw new Error(
          `Unauthorized: This action requires admin or supervisor role. Your user type: ${profile.user_type}`,
        );
      }
    }
    ...
    if (action === "generate_suggestions") {
      ...
      const { data: projects } = await supabaseClient.from("projects").select("*").is("supervisor_id", null);

      const { data: supervisors } = await supabaseClient.from("supervisors").select("*");
      ...
      if (!projects?.length || !supervisors?.length) {
        return new Response(
          JSON.stringify({
            ...
            suggestions_count: 0,
            allocated: 0,
            message: "No unassigned projects or no supervisors available.",
          }),
      ...
      if (supervisorSlots.length === 0) {
        return new Response(
          JSON.stringify({
            ...
            suggestions_count: 0,
            allocated: 0,
            message: "All supervisors at capacity.",
          }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      }
    ...
    if (action === "create_group") {
      ...
      const memberInfo = members.map((m: any) => `${m.full_name.trim()} (${m.reg_number.trim()})`).join(", ");
    ...
    if (action === "allocate_group") {
      ...
      const { data: supervisors, error: supervisorsError } = await supabaseClient.from("supervisors").select("*");
      ...
      for (let si = 0; si < (supervisors || []).length; si++) {
        ...
        if (
          group.project_type &&
          supervisor.research_areas?.some(
            (area: string) =>
      ...
      if (!bestMatch) {
        return new Response(JSON.stringify({ error: "No available supervisors found" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    if (action === "auto_allocate_project") {
      ...
      const supervisorsData = await getSupervisorPool(adminClient, maxProjects);

      const { projectCategory, matchingSupervisors } = getMatchingSupervisors(
        project,
        supervisorsData || [],
        maxProjects,
      );
      ...
      if (!projectCategory || matchingSupervisors.length === 0) {
        ...
        return new Response(
          JSON.stringify({
            ...
            allocated: false,
            message: !projectCategory
              ? "Project category is missing, so expertise matching could not run."
              : "No supervisor with matching expertise was found. Admin has been notified for manual assignment.",
          }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      }
      ...
      if (scoredMatches.length === 0) {
        ...
        return new Response(
          JSON.stringify({
            success: true,
            allocated: false,
            message: "No qualified supervisor with available capacity could be routed.",
          }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      }
      ...
      const supervisorNameMap = new Map(
        scoredMatches.map(({ supervisor }) => [supervisor.user_id, getDisplayName(supervisor, "Matching supervisor")]),
      );
      const supervisorNames = scoredMatches.map(
    ...
    if (action === "approve_project") {
      ...
      if (isSupervisor) {
        ...
        if (!allocationCheck) {
          return new Response(JSON.stringify({ error: "This project was not routed to you for review." }), {
            status: 403,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        ...
        const { data: rules } = await supabaseClient.from("allocation_rules").select("*");
        const maxProjectsDefault = rules?.find((r) => r.rule_name === "max_projects_per_supervisor")?.rule_value || 5;
        const allSupervisors = await getSupervisorPool(adminClient, maxProjectsDefault);
        ...
        if (project && allSupervisors?.length) {
          ...
          if (matchingSupervisors.length === 0) {
            return new Response(
              JSON.stringify({
                error: `No supervisor with matching expertise was found for ${projectCategory}. Please use manual assignment.`,
              }),
              { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
            );
          }
          ...
          if (bestMatch) {
            ...
            assignedSupervisorName = getDisplayName(bestMatch.supervisor, "a matched supervisor");
          }
        }
      }
      ...
      if (assignedSupervisorId) {
        ...
        await syncSupervisorProjectCount(supabaseClient, assignedSupervisorId);
      }
    ...
    if (action === "reject_project_with_feedback") {
      ...
      await supabaseClient.from("pending_allocations").update({ status: "rejected" }).eq("project_id", projectId);
    ...
    if (action === "bulk_auto_allocate") {
      ...
      for (let pi = 0; pi < unassignedProjects.length; pi++) {
        const { matchingSupervisors } = getMatchingSupervisors(
          unassignedProjects[pi],
          allSupervisors || [],
          maxProjectsDefault,
        );
        const matchingSupervisorIds = new Set(matchingSupervisors.map((supervisor: any) => supervisor.user_id));
      ...
      for (const sup of allSupervisors) {
        if (projectCounts[sup.user_id] !== (sup.current_projects || 0)) {
          await syncSupervisorProjectCount(supabaseClient, sup.user_id);
        }
      }
